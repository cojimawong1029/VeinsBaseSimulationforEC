...
bool BaseConnectionManager::isInRange(BaseConnectionManager::NicEntries::mapped_type pFromNic, BaseConnectionManager::NicEntries::mapped_type pToNic)
{
	...
	//getSimulation()->getComponent(pFromNic->hostId)->bubble(getSimulation()->getComponent(pFromNic->hostId)->getNedTypeName());
	if(strcmp(getSimulation()->getComponent(pFromNic->hostId)->getNedTypeName(),getSimulation()->getComponent(pToNic->hostId)->getNedTypeName())==0)return 0;
	//如果两个Nic的父模块是同一个类型：都是车辆，那么他们之间就不会相互连接。
