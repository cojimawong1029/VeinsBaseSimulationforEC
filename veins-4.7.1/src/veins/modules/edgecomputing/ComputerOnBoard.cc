...
#include "veins/modules/edgecomputing/ComputerOnBoard.h"
...
Define_Module(ComputerOnBoard);

ComputerOnBoard::ComputerOnBoard() {
    endTxEvent = nullptr;
}
...
void ComputerOnBoard::initialize() {
    mips = par("mips");
    ram = par("ram");
...
void ComputerOnBoard::changeUsage(int i) {
    switch (i) {
    case 0:
...
void ComputerOnBoard::handleSelfMsg(cMessage *msg) {
...
void ComputerOnBoard::chooseFromQueue(int removeId) {
    if (!queueVMs.empty()) {
        int flag = vmAllocation->chooseFlagVM(queueTasks, PMs, queueVMs,removeId);
        if (flag != -1) {
            ...
            t=tsk->getMi()/dynamic_cast<VM*>(*it)->getMips();
            queueVMs.erase(it);
...
int ComputerOnBoard::createVM(VM* vm) {
    int i;
...
void ComputerOnBoard::handleMessage(cMessage *msg) {
