/*
 * ComputerEdgeMAOCO.cc
 *
 *  Created on: 2019��7��10��
 *      Author: xiaow
 */

#include "veins/modules/edgecomputing/MAOCO/ComputerEdgeMAOCO.h"
...
Define_Module(ComputerEdgeMAOCO);

ComputerEdgeMAOCO::ComputerEdgeMAOCO() {
    endTxEvent = nullptr;
}
...
void ComputerEdgeMAOCO::initialize() {
    mips = par("mips");
    ram = par("ram");
...
void ComputerEdgeMAOCO::changeUsage(int i) {
    switch (i) {
    case 0:
...
double ComputerEdgeMAOCO::getMaxRest(double besto){
...
double ComputerEdgeMAOCO::getOcc(){
    double Q=0;
    for(int i=0;i<numofHost;i++){
...
void ComputerEdgeMAOCO::handleSelfMsg(cMessage *msg) {
...
void ComputerEdgeMAOCO::chooseFromQueue(int removeId) {
    if (!queueVMs.empty()) {
...
int ComputerEdgeMAOCO::createVM(VM* vm) {
    int i;
...
void ComputerEdgeMAOCO::handleTaskRequest(TaskRequest *tsk) {
...
void ComputerEdgeMAOCO::handleMessage(cMessage *msg) {
